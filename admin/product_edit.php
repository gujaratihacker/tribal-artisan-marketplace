...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name) || empty($category) || empty($description) || $price <= 0 || $artisan_id <= 0) {
        ...
        try {
            $stmt = $db->prepare("UPDATE products SET artisan_id=?, name=?, category=?, description=?, price=?, tag=?, is_featured=? WHERE id=?");
            $stmt->execute([$artisan_id, $name, $category, $description, $price, $tag ?: null, $is_featured, $id]);
...
<div class="admin-layout">
  ...
  <div class="main-content">
    <div class="top-bar"><h1>Edit Product: <?php echo e($product['name']); ?></h1></div>
    <div class="content-area">
      <div class="card">
        ...
        <form method="POST" action="">
          <div class="form-group">
            ...
            <select name="artisan_id" required>
              <?php foreach ($artisans as $a): ?>
                <option value="<?php echo $a['id']; ?>" <?php echo $product['artisan_id'] == $a['id'] ? 'selected' : ''; ?>><?php echo e($a['name']); ?></option>
              <?php endforeach; ?>
            </select>
          </div>
          <div class="form-group">
          ...
          <div class="form-group">
            ...
            <select name="category" required>
              <?php foreach ($categories as $key => $label): ?>
                <option value="<?php echo $key; ?>" <?php echo $product['category'] === $key ? 'selected' : ''; ?>><?php echo e($label); ?></option>
              <?php endforeach; ?>
            </select>
          </div>
          <div class="form-group">
          ...
          <div class="form-group">
            <label>Tag</label>
            <input type="text" name="tag" value="<?php echo e($product['tag'] ?? ''); ?>">
          </div>
          <div class="form-group">

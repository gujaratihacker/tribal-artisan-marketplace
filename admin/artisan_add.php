...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name) || empty($location) || empty($specialty) || empty($bio) || empty($phone) || empty($email)) {
        ...
        try {
            $db = getDB();
            $stmt = $db->prepare("INSERT INTO artisans (name, location, specialty, bio, phone, email, is_featured) VALUES (?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([$name, $location, $specialty, $bio, $phone, $email, $is_featured]);
            redirect('artisans.php');
        } catch (PDOException $e) {
            $error = 'Failed to add artisan: ' . $e->getMessage();
        }
    }
}
?>
...
.form-group input, .form-group textarea, .form-group select {
  width: 100%; padding: 12px 16px; border: 1px solid #E8D5B7; border-radius: 12px;
  font-family: 'DM Sans', sans-serif; font-size: 0.95rem; transition: all 0.3s;
}
...
<div class="admin-layout">
  ...
  <div class="main-content">
    <div class="top-bar"><h1>Add New Artisan</h1></div>
      ...
      <div class="card">
        ...
        <form method="POST" action="">
          ...
          <div class="form-group">
            ...
            <input type="text" name="location" required value="<?php echo e($_POST['location'] ?? ''); ?>">
          </div>
          <div class="form-group">
            <label>Specialty *</label>
            <input type="text" name="specialty" required value="<?php echo e($_POST['specialty'] ?? ''); ?>">
          </div>
          <div class="form-group">
            <label>Bio *</label>
            <textarea name="bio" required><?php echo e($_POST['bio'] ?? ''); ?></textarea>
          </div>
          <div class="form-group">
            <label>Phone *</label>

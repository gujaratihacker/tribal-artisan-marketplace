...
if (isset($_GET['approve'])) {
    $id = (int)$_GET['approve'];
    $request = $db->prepare("SELECT * FROM artisan_requests WHERE id = ?");
    $request->execute([$id]);
    $req = $request->fetch();
    
    if ($req) {
        // Create artisan record
        $stmt = $db->prepare("INSERT INTO artisans (name, location, specialty, bio, phone, email) VALUES (?, ?, ?, ?, ?, ?)");
        $stmt->execute([$req['name'], $req['location'], $req['specialty'], $req['bio'], $req['phone'], $req['email']]);
        
        // Update request status
        $db->prepare("UPDATE artisan_requests SET status = 'approved' WHERE id = ?")->execute([$id]);
    }
    
    redirect('artisan_requests.php');
}

if (isset($_GET['reject'])) {
    $id = (int)$_GET['reject'];
    $db->prepare("UPDATE artisan_requests SET status = 'rejected' WHERE id = ?")->execute([$id]);
    redirect('artisan_requests.php');
}

if (isset($_GET['delete'])) {
    $id = (int)$_GET['delete'];
    $db->prepare("DELETE FROM artisan_requests WHERE id = ?")->execute([$id]);
    redirect('artisan_requests.php');
}

$requests = $db->query("SELECT * FROM artisan_requests ORDER BY created_at DESC")->fetchAll();
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<title>Artisan Requests — Admin</title>
...
<div class="admin-layout">
  <div class="sidebar">
    ...
    <div class="sidebar-nav">
      ...
      <a href="artisan_requests.php" class="active"><span class="icon">&#128221;</span> Artisan Requests</a>
  ...
  <div class="main-content">
    <div class="top-bar"><h1>Artisan Registration Requests</h1></div>
        ...
        <table>
          ...
          <tbody>
            <?php if (empty($requests)): ?>
              <tr><td colspan="7" style="text-align: center; padding: 40px; color: #7A6555;">No artisan requests yet</td></tr>
            <?php else: ?>
              <?php foreach ($requests as $req): ?>
              <tr>
                <td><strong><?php echo e($req['name']); ?></strong></td>
                <td><?php echo e($req['location']); ?></td>
                <td><?php echo e($req['specialty']); ?></td>
                <td>
                  <?php echo e($req['phone']); ?><br>
                  <small style="color:#7A6555;"><?php echo e($req['email']); ?></small>
                </td>
                <td><?php echo $req['experience_years']; ?> years</td>
                <td>
                ...
                <td>
                  <div class="actions">
                    <?php if ($req['status'] === 'pending'): ?>
                      <a href="artisan_requests.php?approve=<?php echo $req['id']; ?>" class="btn btn-sm btn-success" onclick="return confirm('Approve this artisan?')">Approve</a>
                      <a href="artisan_requests.php?reject=<?php echo $req['id']; ?>" class="btn btn-sm btn-danger" onclick="return confirm('Reject this artisan?')">Reject</a>
                    <?php endif; ?>
                    <a href="artisan_requests.php?delete=<?php echo $req['id']; ?>" class="btn btn-sm btn-danger" onclick="return confirm('Delete this request?')">Delete</a>
                  </div>
                </td>
              </tr>
              <?php endforeach; ?>
